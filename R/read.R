...
tranform.antrax_output <- function(data) {
  data %>%
    dplyr::group_by(.data$tracking_file) %>%
    dplyr::mutate(frame = row_number()) %>% # add variable of frame
    dplyr::ungroup() %>%
    tidyr::pivot_longer(cols = !.data$tracking_file & !.data$frame, names_to = "colour_axis", values_to = "value") %>% #
    tidyr::separate(col = .data$colour_axis, into = c("colour", "axis"), sep = "_") %>% #
    tidyr::pivot_wider(names_from = .data$axis, values_from = .data$value) %>%
